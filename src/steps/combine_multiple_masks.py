...
class CombineMultipleMasks(TransformerMixin):
    ...
    def combine_multiple_masks(self, mask_path: str) -> None:
        ...
        active_selector = [k in mask_path for k in self.multiple_masks_selector.keys()]
        if bool(active_selector):
            # new_mask_path = mask_path.replace(active_selector[0], "")
            new_mask_path = ""
            if os.path.exists(new_mask_path):
                return
            mask = cv2.imread(mask_path)
            source_color = self.dataset_masks[self.multiple_masks_selector[active_selector]]
            target_color = self.mask_encodings[self.multiple_masks_selector[active_selector]]
            np.place(mask, mask == source_color, target_color)
            for k, v in self.multiple_masks_selector:
