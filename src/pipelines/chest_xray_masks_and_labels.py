...
class ChestXrayMasksAndLabelsPipeline(BasePipeline):
    ...
    dataset_args: DatasetArgs = chest_xray_masks_and_labels
    pipeline_args: PipelineArgs = PipelineArgs(
        img_prefix="MCUCXR",  # prefix of the source image file names
        mask_selector="ManualMask",
        segmentation_prefix="segmentation",  # just in different folder - not present
        multiple_masks_selector={
            "livermask": "liver",
            "lesionmask": "liver_tumor",
        },  # I can differenciate by right/left or 0/1
