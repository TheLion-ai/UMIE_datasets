...
labels_path = os.path.join(
    os.getcwd(), "testing/test_dummy_data/00_kits23/input/kits23.json"
)
expected_output_path = os.path.join(
    os.getcwd(), "testing/test_dummy_data/00_kits23/expected_output"
)
...
def test_kits23_verify_file_tree():
    ...
    if not DatasetTestingLibrary.verify_file_tree(
        expected_file_tree, current_file_tree
    ):
...
def test_kits23_verify_images_correct():
    ...
    expected_file_tree = glob.glob(
        f"{str(expected_output_path)}/**/*.png", recursive=True
    )
    current_file_tree = glob.glob(f"{str(target_path)}/**/*.png", recursive=True)

    if not DatasetTestingLibrary.verify_all_images_identical(
        expected_file_tree, current_file_tree
    ):
...
def test_kits23_verify_jsonl_correct():
    ...
    expected_jsonl_path = glob.glob(
        f"{expected_output_path}/**/**.jsonl", recursive=True
    )[0]
    ...
    if not DatasetTestingLibrary.verify_jsonl_identical(expected_jsonl, current_jsonl):
        pytest.fail(
            "Brain Tumor Progression pipeline created jsonl contents different than expected."
        )
