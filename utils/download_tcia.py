"""Helpers for downloading datasets from TCIA via the NBIA client."""
...
class Dataset:
    """Configuration for a single TCIA dataset download."""
    ...
    def __init__(
        ...
        """Initialize a TCIA dataset download configuration."""
        self.name = name
    ...
    def __repr__(self) -> str:
        """Return a debug representation of the dataset config."""
        return (
            f"Dataset(name={self.name!r}, manifest_src={self.manifest_src!r}, "
...
datasets = [
    ...
    Dataset(name="CMMD_CLINICAL"),
]


def add_padding(input_string: str) -> str:
    """Strip trailing zeros from a series number and re-pad it to a fixed width."""
...
def copy_all_files(src_folder: str, dst_folder: str) -> None:
    """Copy all files (not subfolders) from src_folder to dst_folder."""
...
with open(config_file, "a") as file:
    for dt in datasets:
        ...
        if dt.manifest_src is not None and dt.path is not None:
            ...
            dataframe = nbia.downloadSeries(
                dt.manifest_src,
                input_type="manifest",
                format="csv",
                csv_filename=dt.path + "/" + dt.csv,  # type: ignore[operator]
                path=dt.path + "/manifest_download",
            )
            manifest_src = dt.path + "/out"
            outdir = Path(manifest_src)
            outdir.mkdir(exist_ok=True)
            with open(dt.path + "/" + dt.csv + ".csv", mode="r", newline="") as rfile:  # type: ignore[operator]
